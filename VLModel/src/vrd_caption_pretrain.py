...
from vrd_caption_data import get_loader_vg
        ...
        for epoch in range(epochs):
            ...
            for step_i, batch in enumerate(self.train_loader):
                ...
                if self.args.fp16 and _use_native_amp:
                    with autocast():
                        if self.args.distributed:
                            results = self.model.module.vrd_pretrain_step(batch)
                        else:
                            results = self.model.vrd_pretrain_step(batch)
                else:
                    if self.args.distributed:
                        results = self.model.module.vrd_pretrain_step(batch)
                    else:
