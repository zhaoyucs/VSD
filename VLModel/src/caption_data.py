...
class VRDCaptionFineTuneDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        if 'target_relation_ids' in batch[0]:
            ...
            target_relation_ids = torch.ones(B, TR_W_L, dtype=torch.long) * self.tokenizer.pad_token_id
            assert TR_W_L == S_W_L
        ...
        if 'target_relation_ids' in batch[0]:
            word_mask = target_relation_ids != self.tokenizer.pad_token_id
...
def get_sp_loader(args, split='train', mode='train',
               batch_size=32, workers=4, distributed=False, gpu=0,
               topk=-1):
