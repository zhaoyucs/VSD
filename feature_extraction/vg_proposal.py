...
if __name__ == "__main__":
    ...
    parser.add_argument('--vrdroot', type=str, default='VisualGenome/')
    ...
    print('Load images from', vrd_img_dir)
    print('# Images:', len(list(vrd_img_dir.iterdir())))

    dataset = VRDataset(vrd_img_dir)

    dataloader = DataLoader(dataset, batch_size=args.batchsize,
                            shuffle=False, collate_fn=collate_fn, num_workers=4)

    output_fname = out_dir.joinpath(f'vg_boxes{NUM_OBJECTS}.h5')
    print('features will be saved at', output_fname)

    desc = f'VG_{(NUM_OBJECTS, DIM)}'
